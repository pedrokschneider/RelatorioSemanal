#!/usr/bin/env python3
"""
Bot Discord Universal - Versão para Serviço Windows (.pyw)
"""
...
class DiscordBotUniversalService(win32serviceutil.ServiceFramework):
    _svc_name_ = "DiscordBotUniversalService"
    _svc_display_name_ = "Discord Bot Universal Service"
    _svc_description_ = "Serviço do Bot Discord Universal para Relatórios Semanais"
    ...
    def SvcDoRun(self):
        try:
            logger.info("Iniciando serviço do Bot Universal")
            self.bot = DiscordBotUniversal()
            
            # Executar o bot em loop
            while self.is_alive:
                try:
                    asyncio.run(self.bot.start())
                except Exception as e:
                    logger.error(f"Erro no bot universal: {e}")
                    if self.is_alive:
                        time.sleep(30)  # Esperar 30 segundos antes de reiniciar
                        
        except Exception as e:
            logger.error(f"Erro fatal no serviço universal: {e}", exc_info=True)
